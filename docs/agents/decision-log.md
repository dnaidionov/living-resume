...
- Agent role: Application Engineer
- Decision: Implement the resume-chat scope gate as a two-stage pipeline: deterministic rules allow/block obvious prompts before classifier/retrieval/answer work, ambiguous prompts use a small LLM scope classifier, and standalone ambiguous classifier decisions are cached in a bounded process-local cache.
...
- Scope impact: `lib/ai/chat-service.ts`, `tests/chat-service.test.ts`, `docs/product/prd.md`, `docs/qa/test-plan.md`, `docs/agents/handoffs.md`, `docs/agents/decision-log.md`.
...
- Agent role: AI Systems Architect / QA Evaluations Agent
- Decision: Rerun OpenRouter model analysis against the current `/api/v1/models` catalog and treat `gpt-5-mini` pricing as the comparison ceiling: `$0.25 / 1M input` and `$2.00 / 1M output`.
- Rationale: The earlier benchmark used a stale free-model scope. The current catalog contains 369 models, including 155 strictly cheaper models and 32 zero-price text models, so recommendations needed to be recalibrated.
- Decision: For OpenRouter-only generation, prefer `qwen/qwen3-next-80b-a3b-instruct:free` for the fastest fit/requirements path and `google/gemini-2.5-flash-lite` as the strongest cheap non-free fit/requirements fallback.
- Rationale: Qwen Next completed the Sourgum fit path in hundreds of milliseconds while preserving the strong-fit verdict. Gemini 2.5 Flash Lite was materially slower than Qwen but far faster and more stable than the OpenRouter `gpt-5-mini` baseline and the current GPT OSS rerun.
- Decision: For chat, prefer `openai/gpt-5.4-nano` as the best cheap OpenRouter-hosted candidate, with `google/gemini-2.5-flash-lite` as the best cheap non-OpenAI-family candidate.
- Rationale: `gpt-5.4-nano` and Gemini 2.5 Flash Lite both returned usable answers in roughly two seconds. Gemini 2.0 Flash Lite was faster but terse, and several free chat models still failed provider-side.
