...
- Resume chat must use a two-stage scope gate: deterministic rules allow or block obvious prompts before retrieval/model work, and ambiguous prompts go through a dedicated small scope classifier before evidence retrieval or answer generation.
...
- If the classifier is unavailable, ambiguous prompts should fall back to resume-mode retrieval/answer generation rather than refusing potentially legitimate resume questions; obvious deterministic abuse must still be refused.
...
- URL-based fit analysis is protected by a live external regression fixture stored in `tests/fixtures/url-fit-analysis-cases.json`; the required build-gate set currently covers Waymo, Sourgum, and Motive product-role URLs.
