...
- Embedded execution qualifiers (`gained through`, `developed through`, `acquired by`, `from building`, `based on building`) must block credential evidence and must be normalized as delivery even when no atomic boundary split is available.
