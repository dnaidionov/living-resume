...
- `lib/ai/chat-service.ts` gates chat scope before retrieval: deterministic rules allow/block obvious resume, project, build, unrelated-task, and prompt-injection prompts; ambiguous standalone prompts use the dedicated `classifier` provider task and store the result in a bounded process-local cache before evidence retrieval or answer generation.
...
- OpenRouter model recommendations are benchmark-bound and should be treated as time-sensitive because provider routing, free tiers, and model behavior change:
  ...
  - for the narrow chat-scope classifier, `qwen/qwen3-next-80b-a3b-instruct:free` is the selected free first-pass model because it supports structured responses and was the fastest successful free structured-task candidate in the rerun; it still needs classifier-specific evals before treating quality as proven
...
- Active configured model routing is currently OpenRouter for all AI tasks: chat uses `openai/gpt-5.4-nano`, classifier uses `qwen/qwen3-next-80b-a3b-instruct:free`, fit and requirements use `openai/gpt-oss-120b:free`, and embeddings use `openai/text-embedding-3-small`.
